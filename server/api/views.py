...
class EnrollmentViewSet(viewsets.ModelViewSet):
	...
	def report_by_plan(self, request):
		...
		enrollments = Enrollment.objects.values('plan__name').annotate(
			count=Count('id')
		).order_by('-count')
	...
	def report_by_size(self, request):
		...
		enrollments = Enrollment.objects.values('canine__size').annotate(
			count=Count('id')
		).order_by('-count')
		return Response(list(enrollments))
	...
	def report_by_breed(self, request):
		...
		enrollments = Enrollment.objects.values('canine__breed').annotate(
			count=Count('id')
		).order_by('-count')[:10]
		return Response(list(enrollments))
...
class AttendanceViewSet(viewsets.ModelViewSet):
	...
	def report_by_date(self, request):
		...
		data = attendances.values('date').annotate(
			count=Count('id')
		).order_by('date')
	...
	def report_by_status(self, request):
		...
		data = Attendance.objects.values('status').annotate(
			count=Count('id')
		).order_by('-count')
		return Response(list(data))
